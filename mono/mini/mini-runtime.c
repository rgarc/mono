...
MONO_SIG_HANDLER_FUNC (, mono_sigfpe_signal_handler)
{
	...
	if (!ji) {
		...
		if (mono_dump_start ())
			mono_handle_native_crash ("fpe", &mctx, info);
...
MONO_SIG_HANDLER_FUNC (, mono_crashing_signal_handler)
{
	...
	mono_sigctx_to_monoctx (ctx, &mctx);
	if (mono_dump_start ())
		mono_handle_native_crash (sys_signame [info->si_signo], &mctx, info);
	if (mono_do_crash_chaining) {
		mono_chain_signal (MONO_SIG_HANDLER_PARAMS);
		return;
	}
...
#endif
{
	...
	if (!mono_domain_get () || !jit_tls) {
		if (!mono_do_crash_chaining && mono_chain_signal (MONO_SIG_HANDLER_PARAMS))
			return;
		if (mono_dump_start())
			mono_handle_native_crash ("segv", &mctx, info);
		if (mono_do_crash_chaining) {
			mono_chain_signal (MONO_SIG_HANDLER_PARAMS);
			return;
		}
	...
	if (!ji) {
		...
		if (mono_dump_start ())
			mono_handle_native_crash ("segv", &mctx, (MONO_SIG_HANDLER_INFO_TYPE*)info);
